...
type Config struct {
	...
	Env               map[string]string            `yaml:"env"`     // environment defaults declared at the root namespace
	...
	NamespaceDefaults map[string]string            `yaml:"-"` // namespace -> fully qualified default task declared by that include
	NamespaceEnv      map[string]map[string]string `yaml:"-"` // namespace -> environment defaults declared by that include
...
type Cmd struct {
	...
	Vars        map[string]Var    `yaml:"vars"`
	Env         map[string]string `yaml:"env"` // environment overrides for task sub-calls; ignored on shell commands
}
